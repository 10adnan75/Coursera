...
constexpr double distance(const pair<long long, long long> &a, const pair<long long, long long> &b) {
...
bool less_than_on_x(pair<long long, long long> &a, pair<long long, long long> &b) {
    return (a.first == b.first) ? (a.second < b.second) : (a.first < b.first);
}

bool less_than_on_y(pair<long long, long long> &a, pair<long long, long long> &b) {
    return (a.second == b.second) ? (a.first < b.first) : (a.second < b.second);
}

void merge(vector<pair<long long, long long>> &points, long long low, long long high) {
    if (low >= high) {
        return;
    }
...
double minimal_distance(vector<pair<long long, long long>> &points, long long low, long long high) {
    if (low >= high) {
    ...
    double d = min(minimal_distance(points, low, mid), minimal_distance(points, mid + 1, high));
